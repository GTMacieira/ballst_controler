...
# 	utilizacao_fatura REAL NOT NULL,
...
with open('C:/Users/CSO5007/Desktop/python_projects/ballst_controler/recursos/DB_Lastros_CSV.csv','r') as fin: # `with` statement available in 2.5+
    ...
    to_db = [(['sequencial'], ['novo_sequencial'],['cliente'], ['dt_embarque'],['dt_recebimento'], ['fatura'], ['moeda'], ['val_fatura'],['utilizacao_fatura'], ['val_utilizado_fatura'], ['val_util_convert_brl'], ['val_util_convert_eur'], ['val_util_convert_usd'], ['tp_contrato'], ['sd_due'], ['ch_acesso_re'], ['obs'], ['total_parcial'], ['ultimo_ano'], ['user'], ['register']) for i in dr]

cur.executemany('insert into ballests (sequencial, novo_sequencial, cliente, dt_embarque, dt_recebimento, fatura, moeda, val_fatura, utilizacao_fatura, val_utilizado_fatura, val_util_convert_brl, val_util_convert_eur, val_util_convert_usd, tp_contrato, sd_due, ch_acesso_re, obs, total_parcial, ultimo_ano, user, register) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);', to_db)
con.commit()
con.close()